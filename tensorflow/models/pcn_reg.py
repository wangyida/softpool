...
class Model:
    def __init__(self, inputs, npts, gt, alpha, num_channel):
        ...
        self.canonical, _, _ = self.create_decoder(
            self.create_encoder(self.inputs_can, npts))
        self.loss, self.update = self.create_loss(self.coarse, self.fine, gt,
                                                  alpha)
        self.outputs1 = self.coarse
        self.outputs2 = self.fine
        self.visualize_ops = [
            tf.split(inputs[0], npts, axis=0), self.coarse, self.gt_can,
            self.fine, gt
        ]
        self.visualize_titles = [
            'input', 'coarse output', 'meshes', 'fine output', 'ground truth'
        ]
    ...
    def canon_pose(self, gt, inputs, npts):
        ...
        rot = tmp_v
        gt_can = tf.concat([tf.matmul(gt[:, :, :3], rot), gt[:, :, 3:]],
                           axis=-1)
        inputs_can = tf.concat([
            tf.matmul(f, rot[idx, :, :])
            for idx, f in enumerate(tf.split(inputs, npts, axis=1))
        ],
                               axis=1)
    ...
    def create_decoder(self, features):
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
            coarse = mlp(features,
                         [1024, 1024, self.num_coarse * self.channels])
            coarse = tf.reshape(coarse, [-1, self.num_coarse, self.channels])
            feat_spool = point_softpool(
                coarse, npts_output=self.num_coarse, orders=self.channels)
            coarse = mlp_conv_act(
                feat_spool, [512, 512, 3], act_dim=self.channels)
        ...
        with tf.variable_scope('folding', reuse=tf.AUTO_REUSE):
            ...
            point_feat = tf.tile(
                tf.expand_dims(coarse, 2), [1, 1, self.grid_size**2, 1])
            point_feat = tf.reshape(point_feat,
                                    [-1, self.num_fine, 3 + self.channels])

            global_feat = tf.tile(
                tf.expand_dims(features, 1), [1, self.num_fine, 1])

            feat = tf.concat([grid_feat, global_feat, point_feat], axis=2)

            feat_spool = tf.tile(
                tf.expand_dims(feat_spool, 2), [1, 1, self.grid_size**2, 1])
            feat_spool = tf.reshape(feat_spool,
                                    [-1, self.num_fine, self.channels])
            ...
            fine = mlp_conv_act(
                feat, [512, 512, 3], act_dim=self.channels)  # + center
    ...
    def create_loss(self, coarse, fine, gt, alpha):
        p_coar_feat = tf.nn.softmax(coarse[:, :, 3:3 + self.channels], -1)
        # p_fine_feat = tf.nn.softmax(fine[:,:,3:3+self.channels], -1)
        p_can_feat = tf.nn.softmax(self.canonical[:, :, 3:3 + self.channels],
                                   -1)
        p_coar_samp = tf.reduce_mean(p_coar_feat, [1])
        # p_fine_samp = tf.reduce_mean(p_fine_feat, [1])
        entropy = tf.nn.relu(
            tf.log(self.channels * 1.0) + tf.reduce_mean(
                tf.reduce_sum(p_coar_samp * tf.log(p_coar_samp), [1]), [0]))
        # entropy += tf.nn.relu(tf.log(self.channels*1.0) + tf.reduce_mean(tf.reduce_sum(p_fine_samp * tf.log(p_fine_samp), [1]), [0]))
        ...
        loss_coarse += chamfer(self.canonical[:, :, 0:3],
                               self.gt_can[:, :, 0:3])
        ...
        for i in range(np.shape(gt)[0]):
            ...
            sem_gt = tf.cast(
                tf.one_hot(
                    tf.gather_nd(
                        tf.cast(gt[i, :, 3] * self.channels, tf.int32), index),
                    self.channels), tf.float32)
            loss_sem_coarse = tf.reduce_mean(-tf.reduce_sum(
                0.97 * sem_gt * tf.log(1e-6 + sem_feat) +
                (1 - 0.97) * (1 - sem_gt) * tf.log(1e-6 + 1 - sem_feat), [1]))
            loss_coarse += 0.01 * loss_sem_coarse
        add_train_summary('train/coarse_loss', loss_coarse)
        update_coarse = add_valid_summary('valid/coarse_loss', loss_coarse)

        loss_fine = chamfer(fine[:, :, 0:3], gt[:, :, 0:3])
        _, retb, _, retd = tf_nndistance.nn_distance(fine[:, :, 0:3],
                                                     gt[:, :, 0:3])
        for i in range(np.shape(gt)[0]):
            index = tf.expand_dims(retb[i], -1)
