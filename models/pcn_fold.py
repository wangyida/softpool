...
class Model:
    def __init__(self, inputs, npts, gt, alpha):
        ...
        self.coarse, self.fine, self.fold = self.create_decoder(self.features)
        self.loss, self.update = self.create_loss(self.coarse, self.fine, self.fold, gt, alpha)
        self.outputs = self.fine
        self.visualize_ops = [tf.split(inputs[0], npts, axis=0), self.fine, self.fold, gt]
    ...
    def create_decoder(self, features):
        ...
        with tf.variable_scope('folding2', reuse=tf.AUTO_REUSE):
            fine_feat = tf.concat([fine, point_feat, global_feat], axis=2)

            fold = mlp_conv(fine_feat, [512, 512, 3+11]) 
        return coarse, fine, fold

    def create_loss(self, coarse, fine, fold, gt, alpha):
        ...
        loss_fine = chamfer(fine[:,:,0:3], gt[:,:,0:3]) + chamfer(fold[:,:,0:3], gt[:,:,0:3])
        """
