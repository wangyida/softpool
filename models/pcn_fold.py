...
class Model:
    def __init__(self, inputs, npts, gt, alpha, num_channel):
        ...
        self.fold1, self.fold2, self.mesh, self.entropy = self.create_decoder(
            self.features)
        ...
        self.visualize_ops = [
            tf.split(inputs[0], npts, axis=0), self.fold1, self.fold2, gt
        ]
        self.visualize_titles = ['input', 'fold1', 'fold2', 'ground truth']

    def create_encoder(self, inputs, npts):
        with tf.variable_scope('encoder_0', reuse=tf.AUTO_REUSE):
            features_reg = mlp_conv(inputs[:, :, 0:3],
                                    [128, 256 + self.channels])
            tmp_s, tmp_u, tmp_v = tf.linalg.svd(features_reg[:, :, 0:256])
            features_3d = tf.matmul(
                tmp_u, tf.matmul(tf.linalg.diag(tmp_s), tmp_v, adjoint_b=True))
        ...
        return features, tf.concat(
            [features_3d[:, :, 0:3], features_reg[:, :, 256:]], axis=2)
    ...
    def create_decoder(self, features):
        with tf.variable_scope('fold1_0', reuse=tf.AUTO_REUSE):
            ...
            point_feat = tf.tile(
                tf.expand_dims(features, 1), [1, self.grid_size**2, 1])
            fold1_reg = mlp_conv(
                tf.concat([point_feat, grid_feat], axis=2),
                [512, 512 + self.channels])
        ...
        with tf.variable_scope('fold2', reuse=tf.AUTO_REUSE):
            fold2 = mlp_conv(
                tf.concat([point_feat, fold1], axis=2),
                [512, 512, 3 + self.channels])
        mesh = fold2 + fold1

        p_coar_feat = tf.nn.softmax(
            tf.round(fold1[:, :, 3:3 + self.channels]), -1)
        p_fine_feat = tf.nn.softmax(
            tf.round(fold2[:, :, 3:3 + self.channels]), -1)
