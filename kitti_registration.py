...
def register(source, target, args):
    ...
    result = registration_icp(source, target, args.max_dist, np.eye(4),
                              residual, criteria)
    source_trans = copy.deepcopy(source)
    source_trans.transform(result.transformation)
    R = result.transformation[:3, :3]
    t = result.transformation[:3, 3] + target_center - np.dot(
        source_center, R.T)
...
def track(args):
    ...
    writer.writerow(
        ['id', 'r_err_part', 't_err_part', 'r_err_comp', 't_err_comp'])
    ...
    for filename in os.listdir(args.tracklet_dir):
        ...
        prev_R, prev_t = bbox2rt(
            np.loadtxt(os.path.join(args.bbox_dir, '%s.txt' % car_ids[0])))
        prev_partial = read_point_cloud(
            os.path.join(args.partial_dir, '%s.pcd' % car_ids[0]))
        prev_complete = read_point_cloud(
            os.path.join(args.complete_dir, '%s.pcd' % car_ids[0]))
        for i in range(args.interval, len(car_ids), args.interval):
            n += 1
            frame = int(car_ids[i].split('_')[1])
            instance_id = '%s_frame_%d_to_%d' % (tracklet_id, prev_frame,
                                                 frame)

            R, t = bbox2rt(
                np.loadtxt(os.path.join(args.bbox_dir, '%s.txt' % car_ids[i])))
            R_gt = np.dot(R, prev_R.T)
            t_gt = t - np.dot(prev_t, R_gt.T)

            partial = read_point_cloud(
                os.path.join(args.partial_dir, '%s.pcd' % car_ids[i]))
            R_part, t_part, partial_trans, partial_target = register(
                prev_partial, partial, args)
            ...
            complete = read_point_cloud(
                os.path.join(args.complete_dir, '%s.pcd' % car_ids[i]))
            R_comp, t_comp, complete_trans, complete_target = register(
                prev_complete, complete, args)
            ...
            writer.writerow(
                [instance_id, r_err_part, t_err_part, r_err_comp, t_err_comp])
            ...
            if n % args.plot_freq == 0:
                ...
                plot_pcd_pair(
                    ...
                    (r_err_part, t_err_part), ['Reds', 'Blues'],
                    size=5)
                ...
                plot_pcd_pair(
                    ...
                    complete_target,
                    'Rotation error %.4f\nTranslation error %.4f' %
                    (r_err_comp, t_err_comp), ['Reds', 'Blues'],
                    size=0.5)
                plt.subplots_adjust(
                    left=0, right=1, bottom=0, top=0.95, wspace=0)
                fig.savefig(
                    os.path.join(args.results_dir, 'plots',
                                 '%s.png' % instance_id))
                plt.close(fig)
    print(
        'Using original pcd: average roration error %.4f  average translation error %.4f'
        % (total_r_err_part / n, total_t_err_part / n))
    print(
        'Using completed pcd: average roration error %.4f  average translation error %.4f'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--complete_dir', default='data/results/kitti/pcn_emd/completions')
    parser.add_argument('--bbox_dir', default='data/kitti/bboxes')
    parser.add_argument('--tracklet_dir', default='data/kitti/tracklets')
    parser.add_argument(
        '--results_dir', default='data/results/kitti_registration')
    parser.add_argument(
        '--interval', type=int, default=1, help='number of frames to skip')
    parser.add_argument(
        '--max_iter', type=int, default=100, help='max iteration for ICP')
    parser.add_argument(
        '--max_dist',
        type=float,
        default=0.05,
        help='matching threshold for ICP')
